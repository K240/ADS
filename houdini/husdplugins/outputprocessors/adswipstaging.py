...
class AdsWipStagingOutputProcessor(OutputProcessor):
    ...
    def _staging_for_save(self):
        # Houdini instantiates output processors while building menus and
        # parameter interfaces, before a publish target has been selected.
        # Read ADS_WIP_* only when a save is actually processed.
    ...
    def processSavePath(self, asset_path, referencing_layer_path, asset_is_layer):
        return self._staging_for_save().redirect(asset_path)
